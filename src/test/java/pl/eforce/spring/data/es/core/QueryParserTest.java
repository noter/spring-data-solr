...
import pl.eforce.spring.data.es.core.QueryParser;
...
import pl.eforce.spring.data.es.core.query.SimpleFilterQuery;
...
public class QueryParserTest extends AbstractITestWithEmbeddedElasticSearch {
	...
	public void testConstructESSearchQueryWithFacetSort() {
		...
		String facetPath = "$.facets.facet_1.terms.";
		with(searchRequestBuilder.toString()).assertThat("$.query.bool.must.bool.must.field.field_1", is("value_1"))
				.assertThat(facetPath + "field", is("facet_1")).assertThat(facetPath + "size", is(10)).assertThat(facetPath + "order", is("count"));
